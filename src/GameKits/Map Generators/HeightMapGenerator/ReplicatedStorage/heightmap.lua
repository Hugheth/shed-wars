...
local function diamond(map, x, y, d, f)
	local sum, num = 0, 0
	if 0 <= x - d then
		sum, num = sum + map[x - d][y], num + 1
	end
	if x + d <= map.w then
		sum, num = sum + map[x + d][y], num + 1
	end
	if 0 <= y - d then
		sum, num = sum + map[x][y - d], num + 1
	end
	if y + d <= map.h then
		sum, num = sum + map[x][y + d], num + 1
	end
...
local function diamondsquare(size, f, random)
	-- create map
...
return function(width, height, f)
	-- make heightmap
	return diamondsquare(pot(max(width, height)), f)
end
